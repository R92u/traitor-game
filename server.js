...
io.on('connection', (socket) => {
  ...
  socket.on('game:start', ({ word, traitorId }, cb) => {
    ...
    // Get only non-host players
    const gameParticipants = lobby.players.filter(p => p.socketId !== lobby.hostId);
    if (gameParticipants.length < 2) return cb?.({ ok: false, error: 'Нужно минимум 2 игрока (не считая ведущего)' });

    lobby.word = word;
    lobby.traitorId = traitorId;
    lobby.state = 'revealing';
    // Only non-host players participate in reveal and game
  ...
  socket.on('game:nextTurn', () => {
    ...
    // Only non-host players participate
    const gameParticipants = lobby.players.filter(p => p.socketId !== lobby.hostId && p.alive);
    if (gameParticipants.length === 0) return;
    
    const curr = gameParticipants[lobby.currentTurnIndex % gameParticipants.length];
    
    // Host or current player can end turn
    if (socket.id !== lobby.hostId && socket.id !== curr?.socketId) return;
    
    lobby.currentTurnIndex = (lobby.currentTurnIndex + 1) % gameParticipants.length;
  ...
  socket.on('game:vote', ({ targetId }) => {
    ...
    // Only count non-host alive players
  ...
  socket.on('game:playersWin', () => {
    const lobby = lobbies[socket.data.lobbyCode];
    if (!lobby || socket.id !== lobby.hostId) return;
    // Only non-host players get points
